...
export const fetchNotes = async () => {
  const response = await client.get('/api/notes/get-notes')
  return response.data.data
}

// CREATE note — POST /api/notes/add-note
export const createNoteApi = async (noteData) => {
  const response = await client.post('/api/notes/add-note', noteData)
  return response.data.data
}

// UPDATE note — PUT /api/notes/update-note/:id
export const updateNoteApi = async (id, noteData) => {
  const response = await client.put(`/api/notes/update-note/${id}`, noteData)
  return response.data.data
}

// DELETE note — DELETE /api/notes/delete-note/:id
export const deleteNoteApi = async (id) => {
  const response = await client.delete(`/api/notes/delete-note/${id}`)
